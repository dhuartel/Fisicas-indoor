...
glmReadMTL(GLMmodel* model, char* name)
{
...
#if GLM_TEXTURES_EXTENSION_ENABLED
	AUX_RGBImageRec *TextureImage[1];
#endif //GLM_TEXTURES_EXTENSION_ENABLED
    ...
    for (i = 0; i < nummaterials; i++) {
		...
		model->materials[i].texturemap_index = 0;
#endif // GLM_TEXTURES_EXTENSION_ENABLED
    ...
    while(fscanf(file, "%s", buf) != EOF) {
		...
		case 'm':		//texturemap.
			if (strstr(buf, "map_Kd")) {
				...
				buf[strlen(buf)-2] = 'm';
				buf[strlen(buf)-1] = 'p';
				model->materials[nummaterials].texturemap = strdup(buf+1);

				// Set the pointer to NULL
				memset(TextureImage,0,sizeof(void *)*1);

				// Load the bitmap and assign our pointer to it
				TextureImage[0] = auxDIBImageLoadA(model->materials[nummaterials].texturemap);

				// Generate the OpenGL texture id
				glGenTextures(1, &model->materials[nummaterials].texturemap_index);

				// Bind this texture to its id
				glBindTexture(GL_TEXTURE_2D, model->materials[nummaterials].texturemap_index);
    ...
    if (model->materials) {
		for (i = 0; i < model->nummaterials; i++){
			...
			if (model->materials[i].texturemap){
				glDeleteTextures(1, &model->materials[i].texturemap_index);
        ...
        if (mode & GLM_MATERIAL) {
			...
			if (mode & GLM_TEXTURE) {		
				if (material->texturemap_index) {
					glBindTexture(GL_TEXTURE_2D, material->texturemap_index);
...
/*
#if GLM_TEXTURES_EXTENSION_ENABLED
		if ((mode & GLM_TEXTURE) && material->texturemap_index)
#else
		if (mode & GLM_TEXTURE)
#endif //GLM_TEXTURES_EXTENSION_ENABLED
*/
